...
var (
	RequestInfoNotFoundErr = errors.New(`[ERROR] did not find request.info in curiefense medatada`)
)
...
func (s *grpcServer) parseRawEntry(entry *ald.HTTPAccessLogEntry) (*entities.LogEntry, error) {
	...
	if !ok { /* This log line was not generated by curiefense */
		log.Print(`"[DEBUG] No curiefense metadata => drop log entry"`)
		return nil, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[ERROR] Error unmarshalling metadata json string [%v]: %v", rInfo.GetStringValue(), err)
	}
